...
test_mov = {
  ...
  VerifyTest("T: mov 0xffffffffffffffff rdx")\
  ...
  VerifyTest("T: mov 0x7fffffffffffffff rdx")\
...
test_lea = {
  VerifyTest("T:lea ").bytes(intel(["lea rdi, [rsi - 0x15]"])).tags({"lea"})
...
test_idiv = {
  VerifyTest("T:idiv").bytes(intel(["idiv rsi"])).tags({"min", "idiv"}).
  case(
