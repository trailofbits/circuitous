...
test_adc = {
  ...
  VerifyTest("adc 2 variants").tags({"adc"})
...
test_xor = {
  ...
  VerifyTest("xor 2 variants").tags({"min", "xor"})
...
test_or = {
  ...
  VerifyTest("or 2 variants").tags({"min", "or"})
...
test_rcr = {
  VerifyTest("rcr rax").tags({"min", "rcr"}).bytes(intel(["rcr rax"]))
  .case(I = S(0x100).RAX(0b101).aflags(0), R = True)
  .case(I = S(0x100).RAX(0b1010).aflags(0), R = True),

  VerifyTest("rcr rbx imm8").tags({"min", "rcr"}).bytes(intel(["rcr rbx, 0x5"]))
  .case(I = S(0x100).RBX(0b1011111).aflags(0), R = True)
  .case(I = S(0x100).RBX(0b1000000).aflags(0), R = True),

  VerifyTest("rcr rbx 0").tags({"rcr"}).bytes(intel(["rcr rbx, 0"]))
  .case(I = S(0x100).RBX(0b101).aflags(0), R = True)
  .case(I = S(0x100).RBX(0b101).aflags(1), R = True),

  VerifyTest("rcr rdx cl").tags({"rcr"}).bytes(intel(["rcr rdx, cl"]))
  .case(I = S(0x100).RDX(0x7fffffffffffffff).RCX(63).aflags(0), R = True)
  .case(I = S(0x100).RDX(0x7fffffffffffffff).RCX(65).aflags(1), R = True)
}

test_ror = {
  VerifyTest("ror rax").tags({"ror"}).bytes(intel(["ror rax"]))
  .case(I = S(0x100).RAX(0b101).aflags(0), R = True)
  .case(I = S(0x100).RAX(0b1010).aflags(0), R = True),

  VerifyTest("ror rbx imm8").tags({"ror"}).bytes(intel(["ror rbx, 0x5"]))
  .case(I = S(0x100).RBX(0b1011111).aflags(0), R = True)
  .case(I = S(0x100).RBX(0b1000000).aflags(0), R = True),

  VerifyTest("ror rbx 0").tags({"min", "ror"}).bytes(intel(["ror rbx, 0"]))
  ...
  VerifyTest("ror rdx cl").tags({"ror", "min"}).bytes(intel(["rcl rdx, cl"]))
...
test_rcl = {
  VerifyTest("rcl rax").tags({"min", "rcl"}).bytes(intel(["rcl rax"]))
  .case(I = S(0x100).RAX(0b101).aflags(0), R = True)
  .case(I = S(0x100).RAX(0b1010).aflags(0), R = True),

  VerifyTest("rcl rbx imm8").tags({"min", "rcl"}).bytes(intel(["rcl rbx, 0x5"]))
  .case(I = S(0x100).RBX(0b1011111).aflags(0), R = True)
  .case(I = S(0x100).RBX(0b1000000).aflags(0), R = True),

  VerifyTest("rcl rbx 0").tags({"rcl"}).bytes(intel(["rcl rbx, 0"]))
  .case(I = S(0x100).RBX(0b101).aflags(0), R = True)
  .case(I = S(0x100).RBX(0b101).aflags(1), R = True),

  # In `min` because it tests undefs.
  VerifyTest("rcl rdx cl").tags({"rcl", "min"}).bytes(intel(["rcl rdx, cl"]))
  .case(I = S(0x100).RDX(0x7fffffffffffffff).RCX(63).aflags(0), DE = MS(), R = False)
  .case(I = S(0x100).RDX(0x7fffffffffffffff).RCX(63).aflags(0), DE = MS().uOF(), R = True)
  .case(I = S(0x100).RDX(0x7fffffffffffffff).RCX(63).aflags(1), DE = MS().uOF(), R = True)
  # TODO(lukas): Not sure but 65 % 64 is 1 therefore `OF` should be defined
...
test_all = {
  ...
  VerifyTest("encoding adc, sbb").tags({"min", "reduce"})
...
test_reduce_regs = {
  VerifyTest("mov reg, imm").tags({"reduce_regs"})
  ...
  VerifyTest("mov reg, reg").tags({"reduce_regs"})
  ...
  VerifyTest("add reg, reg").tags({"reduce_regs"})
  ...
  VerifyTest("add reg, imm/reg").tags({"reduce_regs"})
  ...
  VerifyTest("add/mov reg/reg, reg/imm").tags({"reduce_regs", "min"})
...
test_identity_ops = {
  VerifyTest("add rax, rax").bytes(intel(["add rax, rax"])).tags({"reduce_regs", "identity"})
...
test_reg_parts = {
  VerifyTest("mov reg16, reg16").tags({"reduce_regs"})
  ...
  VerifyTest("mov reg16/32, reg16/32").tags({"reduce_regs"})
  ...
  VerifyTest("mov reg32/r8d reg32/r8d - partial").tags({"reduce_regs", "todo"})
  ...
  VerifyTest("mov reg64/32/16 reg64/32/16/imm64/32/16 - partial").tags({"reduce_regs", "todo"})
  ...
  VerifyTest("mov reg32/r8d reg32/r8d - full").tags({"reduce_regs"})
  ...
  VerifyTest("mov rnd rnd - full").tags({"reduce_regs"})
  ...
  VerifyTest("mov rnw rnw - full").tags({"reduce_regs"})
  ...
  VerifyTest("mov rnb rnb - full").tags({"reduce_regs"})
  ...
  VerifyTest("xor rnw rnw - full").tags({"reduce_regs"})
  ...
  VerifyTest("xor rnb rnb - full").tags({"reduce_regs"})
  ...
  VerifyTest("mov al/ah versions").tags({"reduce_regs"})
  ...
  VerifyTest("xor 8 lift").tags({"reduce_regs", "xor"})
  .DI(S(0x250))
  .bytes(intel(["xor al, r8b"])).case(run_bytes = 0, R=True),


  VerifyTest("xor 16 lift").tags({"reduce_regs", "xor"})
  .DI(S(0x250))
  .bytes(intel(["xor ax, r8w"])).case(run_bytes = 0, R=True),

  VerifyTest("xor 32 lift").tags({"reduce_regs", "xor"})
  .DI(S(0x250))
  .bytes(intel(["xor eax, ebx"])).case(run_bytes = 0, R=True),

  VerifyTest("adc al 0x0").tags({"reduce_regs", "todo"})
  .DI(S(0x250))
  .bytes(intel(["adc al, 0x0"]))
  .case(run_bytes = intel(["adc ah, 0xf"]), R=True),


  VerifyTest("xor/adc fractions").tags({"reduce_args", "min"})
  ...
  VerifyTest("mov reg64/32/16 reg64/32/16/imm64/32/16 - full").tags({"reduce_regs"})
  .bytes(intel(["mov rax, rbx", "mov eax, ebx", "mov ax, bx",
                "mov rax, 0x12", "mov eax, 0x12", "mov ax, 0x12",
...
test_addr_ops = {
  VerifyTest("addr op basic").tags({"addr_op", "reduce_regs"})
  ...
  VerifyTest("addr op long displacement").tags({"addr_op", "reduce_regs"})
  ...
  VerifyTest("addr op missing scale").tags({"addr_op", "reduce_regs"})
  ...
  VerifyTest("addr op permutations").tags({"addr_op", "reduce_regs", "min"})
  ...
  VerifyTest("addr lifts 1").tags({"addr_op", "reduce_regs", "lift"})
  .DI(S(0x250))
  .bytes(intel(["lea rax, [rax + rbx]"])).case(run_bytes = 0, R=True),
  VerifyTest("addr lifts 2").tags({"addr_op", "reduce_regs", "lift"})
  .DI(S(0x250))
  .bytes(intel(["lea rax, [rbx + 0xff]"])).case(run_bytes = 0, R=True),
  VerifyTest("addr lifts 3").tags({"addr_op", "reduce_regs", "lift"})
  .DI(S(0x250))
  .bytes(intel(["lea rcx, [rcx + 2 * rdx + 0x5]"])).case(run_bytes = 0, R=True),
  VerifyTest("addr lifts 4").tags({"addr_op", "reduce_regs", "lift"})
  .DI(S(0x250))
  .bytes(intel(["lea rcx, [rax + 0x0]"])).case(run_bytes = 0, R=True),
  VerifyTest("addr lifts 5").tags({"addr_op", "reduce_regs", "lift"})
  .DI(S(0x250))
  .bytes(intel(["lea rax, [rbx + 0xf543f]"])).case(run_bytes = 0, R=True),
  VerifyTest("addr lifts 6").tags({"addr_op", "reduce_regs", "lift"})
  .DI(S(0x250))
  .bytes(intel(["lea rcx, [rcx + 2 * rdx + 0x5fdfd12]"])).case(run_bytes = 0, R=True),
  VerifyTest("addr lifts 7").tags({"addr_op", "reduce_regs", "lift"})
  ...
  VerifyTest("addr op basic r32").tags({"addr_op", "reduce_regs"})
  ...
  VerifyTest("addr op missing scale r32").tags({"addr_op", "reduce_regs"})
  ...
  VerifyTest("addr op reject").tags({"add_op", "reduce_regs"})
  .bytes(intel(["lea eax, [ebx + 2 * ecx + 0x4]"]))
  .DI(S(0xffffffffffff43))
  .case(run_bytes = intel(["lea rax, [rbx + 2 * rcx + 0x4]"]), R=False),

  VerifyTest("addr op permutations r32 test").tags({"addr_op", "reduce_regs"})
  ...
  VerifyTest("tmp").tags({"addr_op", "reduce_regs"})
  ...
  VerifyTest("addr op permutations r32").tags({"addr_op", "reduce_regs", "min"})
  .bytes(intel(["lea eax, [eax + ebx]", "lea eax, [ebx + 0x01]",
  ...
  VerifyTest("addr op permutations r32 reject").tags({"addr_op", "reduce_regs"})
  .bytes(intel(["lea eax, [eax + ebx]", "lea eax, [ebx + 0x01]",
               "lea ecx, [ecx + 2 * edx + 0x5]", "lea ecx, [eax + 0x0]",
               "lea eax, [ebx + 0xf543f]", "lea ecx, [ecx + 2 * edx + 0x5fdfd12]",
               "lea r8d, [ebx + eax]", "lea eax, [r8d + 8 * ebx + 0x7fff]"]))
...
VerifyTest("lea self").tags({"addr_op", "reduce_regs"})
