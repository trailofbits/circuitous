...
namespace circ::mem
{
    ...
    struct Synthetizer
    {
        ...
        Synthetizer( llvm::Value *ts, uint32_t ptr_size )
            : ts( ts ), ptr_size( ptr_size )
        {}
        ...
        {
            auto hint = irops::make_leaf< irops::Memory >( ir,
                                                           irops::memory::size( ptr_size ),
                                                           next );
            ++next;
            return hint;
        }

        auto handle_read( const call_t &c )
        {
            auto [ call, size ] = c;
            ...
            auto coerced_value = [ &, size = size ]()
            {
                check( size <= ptr_size && size > 0 );

                if (size == ptr_size)
                    return out.value();
                return ir.CreateTrunc( out.value(),
                                       ir.getIntNTy( static_cast< uint32_t >( size ) ) );
            }();
        ...
        auto handle_write( const call_t &c )
        {
            auto [ call, size ] = c;
            llvm::IRBuilder<> ir( call );
            ...
            auto coerced_value = [ &, size = size ]()
            {
                ...
                if ( size == ptr_size )
                    return value;
                return ir.CreateZExt( value, ir.getIntNTy( static_cast< uint32_t >( size ) ) );
            }();
    ...
    static inline auto synthetize_memory( llvm::Value *ts, llvm::Value *from,
                                          llvm::Value *to, uint32_t ptr_size )
    {
        return Synthetizer( ts, ptr_size ).all( from, to );
    }

    static inline auto synthetize_memory( llvm::Instruction *from,
                                          llvm::Instruction *to,
                                          uint32_t ptr_size )
    {
