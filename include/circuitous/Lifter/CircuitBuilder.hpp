...
namespace circ
{
    ...
    struct State
    {
        ...
        void commit( llvm::IRBuilder<> &irb );
    ...
    struct CircuitMaker_v2 : has_ctx_ref
    {
        ...
        std::unordered_map< const isem::ISem *, ISemInstance > instances;
        ...
        // ISem  operations
        void materialize( const isem::ISem * );
        void computationals( const isem::ISem * );

        ISemInstance &instance_of( const isem::ISem *def )
        {
            auto it = instances.find( def );
