...
namespace circ::decoder
{
    ...
    {
        auto if_ignore = [ & ]( InputType i )
        {
            return negate( to_val( InputType::ignore ) );
        };
        auto if_not_ignore = [ & ]( InputType i )
        {
            return to_val( i );
        };
    ...
    uint64_t OptionalBitArray< L >::ignored_bits_to_uint64() const
    {
        auto if_ignore = [ & ]( InputType i )
        {
            return to_val( InputType::ignore );
        };
        auto if_not_ignore = [ & ]( InputType i )
        {
            return negate( to_val( InputType::ignore ) );
    ...
    uint64_t OptionalBitArray< L >::convert_to_uint64( auto if_ignore,
                                                       auto if_not_ignore ) const
    {
        uint64_t val = 0;
