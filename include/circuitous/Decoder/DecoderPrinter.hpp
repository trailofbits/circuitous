...
template < int L >
concept SupportedEncodingSize = L <= 64;
...
namespace circ::decoder
{
     ...
     */
    template < int L >
        requires SupportedEncodingSize< L >
    ...
    struct ExtractedDecodeCondition
    {
        ExtractedDecodeCondition( uint32_t low_incl, uint32_t high_excl,
            ...
            low_bit_inc( low_incl ),
            high_bit_exc( high_excl ), bits( bits ) {};
    ...
    struct ExtractedCtx
    {
        ExtractedCtx( const std::string &name, uint8_t size,
            ...
            generated_name( name ),
            encoding_size_in_bytes( size ), decodeConditions( decodeConditions ), vi( vi ) {};

        std::string generated_name;
        uint8_t encoding_size_in_bytes;
        std::vector< ExtractedDecodeCondition >
            decodeConditions; // Extract info we need from this
        VerifyInstruction *vi;
    ...
    {
        decode_context_function_arg( const OptionalBitArray< 64 > &byte, const Var &var ) :
            byte( byte ), var( var ) {};
     ...
     * that contain only the bare minimum of the information to this aspect lightly decoupled of
     * the rest.
     *
     * Afterwards it builds up an Expr type that will be printed through by a simple
     * catamorphism (visitor pattern)
     *
     */
    ...
    private:
        ...
        static std::vector< Expr > top_to_inner_level_args( const Var &array_input,
        ...
        Expr generate_decoder_selection_tree(
        ...
        get_decode_requirements_per_index(
            const std::vector< ExtractedCtx * > &to_split,
