...
// part that will eventually end up in a separate eqsat tool
namespace eqsat::graph {

    using id_t = circ::UnionFind::Id;

    using children_t = std::vector< id_t >;

    struct base {
        using node_pointer = std::shared_ptr< base >;
        using child_type   = id_t;
        ...
        void update_children(gap::invocable auto &&fn) {
...
namespace circ::eqsat {
  ...
  struct ENode : ENodeBase< Storage >
  {
    ...
    bool is_storage_node() const { return std::holds_alternative< StorageNode >( get() ); }
