...
namespace circ::eqsat
{
    ...
    struct Substitution
    {
        ...
        Substitution( std::size_t size ) :
            _mapping( size, Id( 0 ) ), _matched( size, false ) { }
        Substitution( std::size_t size, index idx, Id id ) : Substitution( size )
        {
            set( idx, id );
        }
    ...
    inline std::string name( const anonymous_match & ) { return "anonymous"; }
    inline std::string name( const unary_match &m ) { return label_name( m.label ); }
    inline std::string name( const variadic_match &m ) { return label_name( m.label ); }
    ...
    {
        os << name( m ) << " → {";
        std::copy( std::begin( m.ids ), std::end( m.ids ),
    ...
    inline std::optional< label_match > get_label_match( const matched_labels &match,
                                                         std::string_view label_name )
    {
        auto same_label = [ & ]( const auto &other )
        {
            return name( other ) == label_name;
        };
    ...
    inline std::set< Id > matched_ids( const label_match &match )
    {
        return std::visit(
            overloaded {
                ...
                [ & ]( const variadic_match &m ) -> std::set< Id >
                {
                    return m.ids;
                } },
            match );
    ...
    struct Match
    {
        ...
        Matches result() const
        {
            auto matches = std::visit(
                overloaded { [ & ]( const match_expr &e ) -> Matches { return match( e ); },
                             [ & ]( const expr_list &e ) -> Matches { return match( e ); },
                             [ & ]( const atom &a ) -> Matches { return match( a ); },
                             [ & ]( const bond_expr & ) -> Matches {
                                 throw std::runtime_error(
                                     "bond clause is forbidden in the matching pattern" );
                             },
                             [ & ]( const union_expr & ) -> Matches
                             {
                                 throw std::runtime_error(
                                     "union clause is forbidden in the matching pattern" );
                             } },
                pattern.get() );
    ...
    private:
        ...
        Matches filter_constrained_disjoint_contexts( Matches &&matches,
                                                      const disjoint_expr &constraint ) const
        {
            auto filter = [ & ]( const auto &match )
            {
                return has_disjoint_contexts( match, constraint.contexts );
            };
            matches.erase( std::remove_if( matches.begin(), matches.end(), filter ),
                           matches.end() );
        ...
        Matches filter_constrained_equivalent_places( Matches &&matches,
                                                      const equiv_expr &constraint ) const
        {
            if ( places.empty() )
                return std::move( matches );
            ...
            auto filter_places = [ & ]( const auto &sub )
            {
                return !has_equivalent_places( sub, constraint.places );
            };
            ...
            auto filter_empty_matches = [ & ]( const auto &match )
            {
                return match.substitutions.empty();
            };
            matches.erase(
                std::remove_if( matches.begin(), matches.end(), filter_empty_matches ),
                matches.end() );
        ...
        using named_contexts = std::unordered_map< std::string, std::vector< context_t > >;

        named_contexts get_named_contexts( const anonymous_match &match ) const
        {
            return {}; /* none */
        }
        ...
        {
            return std::visit( [ & ]( const auto &m ) { return get_named_contexts( m ); },
                               match );
        }
        ...
        bool has_disjoint_contexts( const matched_labels &match, const auto &contexts ) const
        {
            ...
            {
                std::visit(
                    overloaded { [ & ]( const single_context &sc )
                                 {
                                     for ( const auto &nodes :
                                           named.at( std::string( sc.ref() ) ) )
                                     {
                                 ...
                                 [ & ]( const variadic_context &vc )
                                 {
                                     for ( const auto &[ name, set ] : named )
                                     {
                                         if ( name.starts_with( '_' )
                                              && name.substr( 1 ).starts_with( vc.ref() ) )
        ...
        bool has_equivalent_places( const Substitution &sub, const auto &con_places ) const
        {
            ...
            return std::all_of( con_places.begin(), con_places.end(),
                                [ & ]( const auto &place )
        ...
        static inline Matches filter_nonunique_matches( Matches &&matches )
        {
                ...
                auto unique = [ & ]( const label_match &lab )
                {
                    return std::visit(
                        overloaded {
                            ...
                            [ & ]( const variadic_match &m )
                            {
                                return std::all_of( m.ids.begin(), m.ids.end(), not_seen );
                            } },
                        lab );
                };
            ...
            matches.erase( std::remove_if( matches.begin(), matches.end(), nonunique ),
                           matches.end() );
            return std::move( matches );
        }
        ...
        static inline Matches filter_commutative_matches( Matches &&matches )
        {
            ...
            matches.erase( std::remove_if( matches.begin(), matches.end(), commutes ),
                           matches.end() );
            return std::move( matches );
        }

        static inline std::optional< matched_labels > merge( const matched_labels &lhs,
                                                             const matched_labels &rhs )
        {
        ...
        Matches match( const auto &e, variadic_label l ) const
        {
            ...
            for ( std::size_t pick = matched.size(); pick > 0; --pick )
            {
                ...
                for_combination( matched.size(), pick,
                                 [ & ]( auto comb )
                                 {
        ...
        {
            return std::visit(
                overloaded { [ & ]( const atom &a ) -> Substitutions
                             { return match_enode( enode, a ); },
                             [ & ]( const expr_list &l ) -> Substitutions
                             { return match_enode( enode, l ); },
                             [ & ]( const match_expr & ) -> Substitutions {
                                 throw std::runtime_error(
                                     "match clause is forbidden in the nested expression" );
                             },
                             [ & ]( const union_expr & ) -> Substitutions {
                                 throw std::runtime_error(
                                     "union clause is forbidden in the nested expression" );
                             },
                             [ & ]( const bond_expr & ) -> Substitutions
                             {
                                 throw std::runtime_error(
                                     "bond clause is forbidden in the nested expression" );
                             } },
                e.get() );
        }
        ...
        {
            return std::visit(
                [ & ]( const auto &v ) -> Substitutions { return match_atom( enode, v ); }, a );
        }
        ...
        static inline std::optional< Substitution > merge_subsitutions( auto lhs,
                                                                        const auto &rhs )
        {
        ...
        static inline Substitutions product_of_substitutions( const Substitutions &lhs,
                                                              const Substitutions &rhs )
        {
        ...
        static inline Substitutions
        product_of_substitutions( const Substitutions &head,
                                  const std::vector< Substitutions > &tail )
        {
    ...
    struct Apply
    {
        Apply( Graph &graph, const Builder &builder, const Pattern &pattern,
               const Places &places ) :
            egraph( graph ),
            builder( builder ), pattern( pattern ), places( places )
        {
        }
        ...
        void apply( const expr &e, const Matches &matches ) const
        {
            std::visit(
                overloaded {
                    [ & ]( const atom &a ) {
                        throw std::runtime_error(
                            "rewrite rule is applied on the level of expression lists" );
                    },
                    ...
                    {
                        throw std::runtime_error(
                            "match clause is forbidden in the rewrite pattern" );
                    } },
                e.get() );
        }
    ...
    struct Rule
    {
        Rule( const std::string &name, const std::string &lhs_, const std::string &rhs_ ) :
    ...
    static inline std::optional< std::string_view > parse_ruleset_name( std::string_view line )
    {
        if ( !line.starts_with( '[' ) )
        {
            return std::nullopt;
    ...
    static inline std::optional< RewriteRule > parse_rule( std::string_view name_line,
                                                           std::istream &is )
    {
    ...
    struct BasicRulesScheduler
    {
        ...
        void apply_rule( Graph &egraph, const Rule &rule ) const { rule.apply( egraph ); }
        ...
        for ( const auto &edge : adviced )
        {
            adviced_bonds(
                ...
                [ & ]( const Node *bond )
                {
                    enumerate(
                        bond->children(),
                        [ & ]( auto bonded_idx, auto bonded_eclass )
                        {
                            for ( const auto &bonded : egraph.eclass( bonded_eclass ).nodes )
                            {
                                ...
                                hook_to_contexts(
                                    ...
                                    [ & ]( auto ctx )
                                    {
                                        ...
                                        if ( auto it = std::find( children.begin(),
                                                                  children.end(), constraint )
                                                       == children.end() )
                                        {
