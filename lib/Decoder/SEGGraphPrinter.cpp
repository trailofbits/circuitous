...
namespace circ::decoder
{
    ...
    void SEGGraphPrinter::print_semantics_emitter()
    {
        for ( auto &node : gap::graph::dfs< gap::graph::yield_node::on_close >( seg_graph ) )
        {
            ...
            {
                auto fd = *func;
                std::cout << "// called externally: " << fd.first.isRoot
                          << " hash: " << fd.first.get_hash() << std::endl;
                ep.print( fd.second );
                std::cout << std::endl;
            }
        }
     ...
     *      the variable name of an lvalue containing the result of the execution of n together
     * with it's subtrees the "setup"/preable code that needs to be executed before the lvalue
     * can be accessed
     *
     * If a node is costly enough that re-use is warrented we declare the preamble as a function
     * decl and add the function to `func_decls`, the lvalue returned will call this newly
     * generated function. this function checks whether `func_decls` already include such a
     * function or not.
     */
        ...
        std::vector< decoder::Var > arg_names )
    {
        ...
        local_vars.push_back(
            operation ); // requires to be first in the list when calling the visitor
        for ( auto &c : node._nodes )
        {
            ...
            auto [ lval, set ] = expression_for_seg_node( func_decls, unique_names_storage, *c,
                                                          child_arg_names );
        ...
        /*
         * we either return a list of statements through a func decl, or just a single one for
         * the current node
         */
        if ( !node.fd )
         ...
         * inside the newly created function, and the caller of the function should not
         * duplicate that
         */
        setup.clear();
    ...
    struct ToExpressionWithIsomorphicSelectsVisitor
        : Visitor< ToExpressionWithIsomorphicSelectsVisitor >
    {
        explicit ToExpressionWithIsomorphicSelectsVisitor( DecodedInstrGen &dig ) : dig( dig )
        {
        }
        ...
        void visit( Select *op )
        {
            ...
            FunctionCall helper_func_call = FunctionCall(
                func.second.function_name, { inner_func_arg1, inner_func_arg2 } );
    ...
    private:
        void register_new_variable( std::size_t index, Var &new_constr_arg, bool is_tuple,
                                    Tuple &tuple )
        {
    ...
    /*
     * This adds all required expressions to express an projection of key location in
     * proj_groups. This assumes that
     ...
     * 2. these variables are always used in calls to semantic dispatching functions in the same
     * way
     *
     ...
     */
    void DecodedInstrGen::expr_for_proj_with_const_sel_loc( projection_maps &proj_groups,
                                                            Operation *key )
    {
     ...
     * As there might have been select choices made during decode time inside the subtree of the
     * circIR root for this projection.
     *
     *
     ...
     *  which would result in non-isomorphic trees. The variable names are always freshly
     * created. Resulting in a lot more variable names created then probably needed.
     */
    ...
    void SEGGraphPrinter::generate_function_definitions()
    {
        ...
        for ( auto vi : circuit->attr< circ::VerifyInstruction >() )
        {
            ...
            {
                auto argument_names = name_storage.get_n_var_names(
                    node->subtree_count, Type( "const VisInputType &" ) );
     ...
     *      If this is not the case than the next variable we write to will be dependent on a
     * decode time value
     *  2. The emission of a select choice may not be dependent on another.
     *      For instance lets say we have 3 select choices A, B and C. With A being a parent to
     * both and B and C have 2 children that are also leafs This example should produce one
     * field/variable but the value it gets is dependent on A regardless.
     *
     * For this it suffices to just check 2 things:
     *  1. No select is a child of another select
     *  2. All nodes use the same SEG Node
     */
    bool DecodedInstrGen::selects_emission_locations_are_constant(
        const std::multimap< Operation *, seg_projection > &proj_groups, Operation *key )
    {
    ...
    {
        /*
         * There are multiple operations which are considered we would need to emit semantics
         * for However some of them might require a decode-time choice. We keep track of the
         * roots in both a set and multimap since multimap does not expose a nice interface to
         * just loop over the keys
         */
