...
namespace circ::decoder
{
    ...
    {
        DecodedInstrGen dig( &seg_graph_printer, select_emission_helper, ctx.vi,
                             ctx.generated_name );
        return dig.create_struct();
    }

    std::vector< OptionalBitArray< 8 > >
    ExtractedCtx::convert_circIR_to_input_type_array() const
    {
        ...
        for ( std::size_t i = 0; i < MAX_ENCODING_LENGTH / 8; i++ )
        {
            ...
            {
                val[ j ]
                    = get_input_type_at_index( static_cast< std::size_t >( ( i * 8 ) + j ) );
            }
    ...
    void DecoderPrinter::extract_ctx()
    {
        ...
        for ( auto &vi : contexts )
        {
            ...
            extracted_ctxs.emplace_back( "DecodedInst_" + std::to_string( vi->id() ),
    ...
    Expr DecoderPrinter::create_top_level_function()
    {
        ...
        auto tree = generate_decoder_selection_tree(
            to_split, std::vector< std::pair< std::size_t, int > >(), 0 );
        fdb.body_insert( tree );

        circ::log_dbg() << "max depth during gen: " << max_depth
                        << " for size of: " << std::to_string( extracted_ctxs.size() );
    ...
    std::vector< Expr > DecoderPrinter::top_to_inner_level_args( const Var &array_input,
                                                                 const Var &arg, size_t offset )
    {
        std::vector< Expr > st;
        ...
        {
            auto indexdVar
                = IndexVar( array_input, Int( static_cast< int64_t >( offset * 8 + i ) ) );
            auto rhs = CastToUint64(
                Shfl( CastToUint64( indexdVar ), Int( static_cast< const int >( 8 * i ) ) ) );
    ...
    /*
     * This function is meant to find the optimal amount of checks required to identify an
     * encoding Instead of calling every <decode_encoding> function once, we do a BST like
     * search to first find the best closest candidate and calling that.
     *
     ...
     * and leaving it naively like this gives decent results (depth 17 instead of the optimal
     * 12)
     *
     * How it works:
     *
    ...
    Expr DecoderPrinter::generate_decoder_selection_tree(
        ...
        std::vector< std::pair< std::size_t, int > > already_chosen_bits, int depth )
    {
        ...
        if ( to_split.size() == 1 )
        {
            ...
            return get_decode_context_function_body( get_decode_context_function_args( ctx ),
                                                     decoded_instr_call );
        }
        ...
        auto candidate_index
            = static_cast< std::size_t >( std::distance( indice_values.begin(), max ) );
        ...
        auto lhs_split
            = generate_decoder_selection_tree( ones, already_chosen_bits, depth + 1 );
        auto rhs_split
            = generate_decoder_selection_tree( zeros, already_chosen_bits, depth + 1 );
    ...
    auto DecoderPrinter::get_decode_context_function_args( const ExtractedCtx &ectx )
        -> decode_func_args
    {
