...
namespace circ
{
    ...
    auto unit_lifter::exalt( unit_t &unit ) -> exalted_value_buckets
    {
        ...
        auto as_buckets = to_buckets( freeze< std::vector >( values ) );

        // TODO( next ): Should something happen here?
        //               Maybe `b_ctx.exalt_epilogue( unit )`?
        auto ctx = make_context( extract( as_buckets, place::ctx ) );
        as_buckets[ place::root ].emplace( ctx );
        return as_buckets;
    }
    ...
    void circuit_producer::finalize()
    {
        ...
        [[ maybe_unused ]] auto vals = pucs.get_isem_lifter().finalize_circuit();
        // Cleanup
