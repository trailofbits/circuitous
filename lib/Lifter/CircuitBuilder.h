...
namespace circuitous {
  ...
  struct SelectFolder : private intrinsics::Select {
    ...
    llvm::CallInst *fetch_complete() {
      for (auto &[_, selects] : ctx_to_selects) {
        LOG(INFO) << selects.size();
        for (auto &select : selects) {
          auto callee = select->getCalledFunction();
          auto [size, _] = intrinsics::Select::ParseArgs(callee);
          CHECK(size == 3ul || size == 4ul);
          if (size == 4ul && intrinsics::Select::is_complete(select)) {
            return select;
          }
        }
      }
    ...
    bool total_compatibility(llvm::CallInst *complete) {
      CHECK(complete);
      LOG(INFO) << "Blueprint: " << remill::LLVMThingToString(complete);
      for (auto &[_, selects] : ctx_to_selects) {
        for (auto &select : selects) {
          LOG(INFO) << remill::LLVMThingToString(select);
          CHECK(are_compatible(complete, select));
        }
      }
      return true;
    }
    ...
    std::size_t required_node_count() {
      std::size_t count = 0;
      for (auto &[_, selects] : ctx_to_selects) {
        count = std::max(count, selects.size());
      }
      return count;
    }

    void generate_templates(llvm::CallInst *blueprint) {
      ...
      for (std::size_t i = 0; i < required_node_count(); ++i) {
        ...
        nodes.emplace_back(llvm::dyn_cast<llvm::CallInst>(args[0]), select);
      }
    }
    ...
    void use_templates() {
      for (auto &[ctx_, selects] : ctx_to_selects) {
        ...
        for (std::size_t i = 0; i < selects.size(); ++i) {
          ...
          auto selector = intrinsics::Select::selector(selects[i]);
          auto [bits, _] = intrinsics::Select::ParseArgs(selects[i]->getCalledFunction());
          if (bits == 3) {
            selector = intrinsics::make_concat(ir, {selector, ir.getInt1(0)});
          }
          auto [hint, node] = nodes[i];
    ...
    void run() {
      auto complete = fetch_complete();
      total_compatibility(complete);
      generate_templates(complete);
      use_templates();
    }
  };
