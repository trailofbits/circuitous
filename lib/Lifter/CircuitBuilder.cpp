...
namespace circ
{
    ...
    void State::commit( llvm::IRBuilder<> &irb )
    {
        log_info() << "[state]: commit";
        irops::make< irops::Commit >( irb, state, 1u );
        log_info() << "[state]:   commited.";
    ...
    {
        auto irb = mk_irb();
        return state->commit( irb );
    }

    void CircuitMaker_v2::materialize( const isem::ISem *def )
    {
        ...
        commit_state();
        instances.emplace( def, ISemInstance( def, inst_size ) );
    ...
    {
        for ( const auto &info : batch.get() )
            isems.make( info.rinst().function, *ctx.module() );

        log_info() << "[cmv2]:" << isems.to_string();

        for ( auto def : isems.all() )
        {
            materialize( def );
            computationals( def );
        }
