    ...
    namespace
    {
        struct AdviceLifter
        {
          ...
          private:
            ...
            void make_arg( llvm::Value *arg, std::size_t idx )
            {
                if ( auto ptr_type = llvm::dyn_cast< llvm::PointerType >( arg->getType() ) )
                {
                    //auto raw_type = ptr_type->getElementType();
                    auto val = irops::make_leaf< irops::ISemDstArg >( irb, ptr_type, idx );
            ...
            void make_prologue()
            {
                ...
                for ( std::size_t i = 2u; i < sem->arg_size(); ++i )
                    make_arg( sem->getArg( static_cast< unsigned int >( i ) ), i - 2 );
            }
